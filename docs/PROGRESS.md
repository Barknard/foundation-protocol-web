...
**Verification:** `_daysim.js` 23 scenarios / 0 failures + `_daysim-fixes.js` 15/15 (S10 now also covers the close-up figure branch); Lane E proved the new scenarios' teeth via 4 sabotage passes on scratch copies (each broke exactly its assertions). Live e2e pass logged below when complete. SW cache `thp-shell-v2.3.0`.
