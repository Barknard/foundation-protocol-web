...
## 2026-08-17 — Foot-pain drill-down + research-backed rehab (v2.3.0)

Eddie's ask: foot pain gets a "where on the foot" drill-down, research-backed exercises for ball-of-foot pain and plantar fasciitis, the program adapts until genuine recovery, full e2e + sim audit. Built research-first on branch `foot-pain-rehab` ([PR #1](https://github.com/Barknard/foundation-protocol-web/pull/1)): evidence review (`docs/EVIDENCE-FOOT.md`) → batched design decisions → spec → plan with a pinned contract → five parallel lanes, one writer per file.

- **New `js/foot.js` domain module:** 5-zone routing (`footKind`), per-condition windows (PF 84d / forefoot 42d ceilings — backstops, not schedules), recovery-gate definitions, guidance cards, the tappable foot-map SVG, and two hand-authored 2-frame foot close-ups (the skeleton has no toe segment).
- **Engine:** `applyCheck` gains an additive `foot` param; `injury.kind` (written-but-never-read since forever) is now real; recovery is **criteria-gated** (pain-free walking + morning first-step pain resolved + the key exercise clean), then a 35-day light "tail" (2×/week on `dayInWeek` ordinals 0/3 — the program pointer, not calendar weekdays); `currentDayPlan()` swaps the mobility slot for the rehab block and substitutes enumerated run blocks (rest in protect, low-impact in ease). Non-foot injuries byte-identical — pinned by a regression sim.
- **Screens:** drill-down (side tap → zone picker → ball-only neural question → 4-chip red-flag screener), gate chips on the pain-free re-check, result-screen protocol/guidance blocks, Today clinician nudge at day ≥14, `dayWhy()` foot branch, Library "Foot" category. The red-flag screener is the first real consumer of the previously-dead `INJURY_FLAG` clinician routing.
- **Exercises (evidence-audited copy):** Towel Heel Raise (Rathleff protocol — presented as *complementary* to the stretch, never "best-evidenced": the 3-month FFI edge did not persist at 6/12 months and the 2021 BJSM core tier omits strengthening), Plantar Fascia Stretch (DiGiovanni 10s×10×3/day), Foot Core (short-foot + towel curls, honestly framed thin), calf stretch reused. Toes/top-of-foot get **guidance cards, not invented protocols** (gout/bunion/turf-toe and the navicular stress-fracture zone are not load-management problems — Eddie's five-zone requirement met with triage honesty).
- **Fresh-research verification pass (Eddie-mandated):** Rathleff/DiGiovanni protocols verified against primary PDFs; two corrections (no significant group difference at 1/6/12mo; DiGiovanni 2-year was a crossover convergence, not a maintained advantage); JOSPT 2023 CPG letter grades remain unreachable (403 ×3) — tiering used, literal grades not claimed.

**Verification:** `_daysim.js` 23 scenarios / 0 failures + `_daysim-fixes.js` 15/15 (S10 now also covers the close-up figure branch); Lane E proved the new scenarios' teeth via 4 sabotage passes on scratch copies (each broke exactly its assertions). **Live e2e (2026-08-17, clean SW, 390×844):** cold boot on v2.3.0 → onboarding → hurt check-in → right-foot tap swaps to the 5-zone map (all targets ≥48px measured, live breadcrumb, back-ghost preserves answers) → ball → neural question → 4-chip safety screener → `foot-meta` with exact 3d/42d windows and `footZone` on the check row → result renders the forefoot protocol under its plain-language name → Today swaps in Forefoot Rehab (walk block correctly kept, lightened) with the foot-aware "why" → recovery gate FAIL branch (one criterion no → still injured, "keep going" copy) and PASS branch (cleared, 35-day tail armed, "Recovery gate passed") → non-foot regression (shoulder → `acute`, 3d/10d, no drill-down/screener/gate, bare clear) → all four tabs render, Library "Foot" category lists all three exercises, close-up figures route for exactly `pf_stretch`/`foot_intrinsic`, skeleton pose routes for `pf_heel_raise`. **Zero console errors across the entire run.** Noted behaviors: a new injury of any kind cancels an in-flight relapse tail (documented Lane-B judgment call); the onboarding Numbers step requires body weight while its copy says "leave any blank" (pre-existing, minor). Eyes-on residuals for a human pass: close-up cross-fade look, foot-map art, banner placement.
